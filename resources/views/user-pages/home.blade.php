  ...
  <script type="text/javascript">
    var for_sig_table = $("table#for_sig_table").DataTable({
      ...
      columns: [
        ...
        {data: "end_time"},
        {data: function(){
          return '<i style = "cursor:pointer;" id = "remove_ot" class="fa fa-remove text-xs" aria-hidden="true"></i>';
        }},
      ]
    });
    ...
    $(document).on('click', 'a#fourth' ,function(){
      for_sig_table.ajax.url("{{route('user.ot_lvl1')}}?stat=3",).load();
    });

    $(document).on('click', 'table#for_sig_table tbody tr i#remove_ot' ,function(){
      var data =  for_sig_table.row( $(this).parent() ).data();
      var id =  data.id;
      Swal.fire({
        title: 'Are you sure?',
